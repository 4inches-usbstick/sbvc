...
def senddisconnectcommand(obj):
    print('Disconnecting...')
    time.sleep(1)
    obj.sendto(b'DCONECT ' + fileget('ids.txt'))

idno = filegeta('id.txt')
...
if getsetting('pushpysendcmd') == 'YES':
    print('Issuing a command...')
    sock.sendto(bytes('CONNECT '+netsetting('myip')+' '+netsetting('udplistenport')+' '+idno+' '+cbedata.get_offline(inicfg, 'main-chatbox-dialto', 'val'), 'utf-8'), (UDP_IP, UDP_PORT) )
    print(bytes('CONNECT '+netsetting('myip')+' '+netsetting('udplistenport')+' '+idno+' '+cbedata.get_offline(inicfg, 'main-chatbox-dialto', 'val'), 'utf-8'), (UDP_IP, UDP_PORT) )
#waiting = True
#while waiting:
 #   s = sock.recvfrom(600)
  #  print(so)
...
import atexit
atexit.register(lambda: senddisconnectcommand(sock))
...
while True:
    ...
    if rms > ambientrms + int(getsetting('rmsthreshold')):
        print('rms threshold exceeded, pushing to:'+(cbedata.get_offline(filegeta('config.cbedata'), 'main-chatbox-dialto', 'val')))
        print(cbedata.get_offline(filegeta('config.cbedata'), 'main-chatbox-dialto', 'val')+filegeta('id.txt'))
        sock.sendto(bytes(cbedata.get_offline(filegeta('config.cbedata'), 'main-chatbox-dialto', 'val')+filegeta('id.txt'), 'UTF-8') + MESSAGE, (UDP_IP, UDP_PORT)) #change dialto here
    else:
